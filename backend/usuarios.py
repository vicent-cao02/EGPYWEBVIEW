...
def autenticar_usuario(username, password, max_intentos=5, bloqueo_min=15):
    ...
    try:
        ...
        if row["bloqueado_hasta"]:
            bloqueado_hasta = datetime.fromisoformat(row["bloqueado_hasta"]) if isinstance(row["bloqueado_hasta"], str) else row["bloqueado_hasta"]
        ...
        cursor.execute("""
            ...
            WHERE username = ?
        """, (intentos, bloqueado.isoformat() if bloqueado else None, username))
        conn.commit()
    finally:
        conn.close()
...
def cambiar_password(username, new_password, actor=None):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE usuarios SET password = ?, requiere_cambio_password = 0 WHERE username = ?",
            (hashed, username)
        )
        conn.commit()
    finally:
...
def set_estado_usuario(username, activo: bool, actor=None):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("UPDATE usuarios SET activo = ? WHERE username = ?", (1 if activo else 0, username))
        conn.commit()
    finally:
        conn.close()
    ...
    return [
        {
            ...
            "requiere_cambio_password": bool(r["requiere_cambio_password"])
        }
        for r in rows
    ]
